import * as React from "react";
import { Meta, StoryObj } from "@storybook/react";
import { AlertTriangle, Info, Megaphone, XCircle } from "lucide-react";

import { Alert } from "@fucina/ui";

const meta: Meta<typeof Alert> = {
  title: "Components/Alert",
  component: Alert,
  tags: ["autodocs"],
  argTypes: {
    variant: {
      control: "select",
      options: ["default", "brand", "danger", "warning"],
      description: "Define the type of message communicated by the alert",
      table: {
        defaultValue: { summary: "default" },
        type: { summary: null },
      },
    },
  },
  parameters: {
    docs: {
      description: {
        component: "Displays a callout for user attention.",
      },
    },
  },
};

export default meta;

type Story = StoryObj<typeof Alert>;

export const Default: Story = {
  render: (args) => (
    <Alert {...args} title="Notice" icon={Megaphone}>
      This is a general notification for your attention. It provides neutral information or basic reminders
      for everyday tasks.
    </Alert>
  ),
  args: {
    variant: "default",
  },
};

export const Brand: Story = {
  render: (args) => (
    <Alert {...args} title="Did you know?" icon={Info}>
      Here&apos;s some useful information or context about the current process or section that might be
      helpful to you.
    </Alert>
  ),
  args: {
    variant: "brand",
  },
};

export const Danger: Story = {
  render: (args) => (
    <Alert {...args} title="Error Detected!" icon={XCircle}>
      Critical issue encountered! Please address this error immediately to prevent system failure or data
      loss.
    </Alert>
  ),
  args: {
    variant: "danger",
  },
};

export const Warning: Story = {
  render: (args) => (
    <Alert {...args} title="Caution Advised" icon={AlertTriangle}>
      Be cautious! There are certain risks associated with this action. Review and proceed with care.
    </Alert>
  ),
  args: {
    variant: "warning",
  },
};
