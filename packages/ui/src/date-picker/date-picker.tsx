...
import { cn, focusInput, focusRing, hasErrorInput } from "@fucina/utils";
        ...
        return (
          <li key={index} className="sm:w-full sm:py-px">
            <button
              ...
              className={cn(
                // base
                "relative flex h-9 w-full items-center truncate rounded border px-2.5 py-1.5 text-left shadow-sm outline-none transition-all sm:border-none sm:shadow-none",
                // text color
                "text-description",
                // border color
                "border-default",
                ...
                {
                  "bg-item-active text": matchesCurrent(preset),
                }
              )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-start">
            {presets && presets.length > 0 && (
              <div
                className={cn(
                  ...
                  "border-default border-b sm:border-b-0 sm:border-r",
                  "overflow-auto"
                )}>
                <div className="absolute px-2 sm:inset-0 sm:py-2">
            ...
            <div>
              ...
              <div className="border-default flex items-center gap-x-2 border-t p-3">
                ...
                <Button variant="primary" size="small" className="w-full" type="button" onClick={onApply}>
...
const RangeDatePicker = ({
  ...
  const onCancel = () => {
    ...
    setEndTime(
      initialRange?.to ? new Time(initialRange.to.getHours(), initialRange.to.getMinutes()) : new Time(0, 0)
    );
    setOpen(false);
  };
          ...
          <div className="flex flex-col overflow-x-scroll sm:flex-row sm:items-start">
            {presets && presets.length > 0 && (
              <div
                ...
                <div className="absolute px-3 sm:inset-0 sm:p-2">
            ...
            <div className="overflow-x-scroll">
              <CalendarPrimitive
                ...
                classNames={{
                  months: "flex flex-row divide-x divide-default",
                }}
                {...props}
              />
              {showTimePicker && (
                <div className="border-default flex items-center justify-evenly gap-x-3 border-t p-3">
                  <div className="flex flex-1 items-center gap-x-2">
                    <span className="text-description">{translations?.start ?? "Start"}:</span>
                    <TimeInput
                      value={startTime}
                  ...
                  <Minus className="stroke-icon size-4 shrink-0 stroke-2" />
                  <div className="flex flex-1 items-center gap-x-2">
                    <span className="text-description">{translations?.end ?? "End"}:</span>
                    <TimeInput
                      value={endTime}
              ...
              <div className="border-default border-t p-3 sm:flex sm:items-center sm:justify-between">
                <p className="text tabular-nums">
                  <span className="text-description">{translations?.range ?? "Range"}:</span>{" "}
      ...
      if ("dateRange" in preset) {
        ...
        if (fromMonth) {
          ...
          if (presetMonth && presetMonth < fromMonth.getMonth()) {
            throw new Error(
              `Preset ${preset.label}'s 'from' is before fromMonth ${format(fromMonth, "MMM, yyyy")}.`
            );
          }
        }
